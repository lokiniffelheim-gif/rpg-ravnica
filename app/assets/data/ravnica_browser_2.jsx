...
function Field({value, onChange, multi=false, accent, ph="Editar..."}) {
  ...
  if (!ed) return (
    <div className="rv-editable" onClick={()=>setEd(true)} style={{position:"relative"}}>
      <span style={{position:"absolute",top:4,right:6,fontSize:16,color:GOLD,opacity:0.65,pointerEvents:"none"}}>✎</span>
      {value
        ? <p style={{margin:0,fontSize:16,lineHeight:1.75,color:"#e8dfc8",whiteSpace:"pre-line",paddingRight:12}}>{value}</p>
        : <p style={{margin:0,fontSize:16,color:"#9a8a70",fontStyle:"italic"}}>{ph}</p>}
    </div>
  );
  return (
    <div>
      {multi
        ...
        : <input autoFocus className="rv-input" value={draft} onChange={e=>setDraft(e.target.value)}
            ...
            style={{borderColor:(accent||GOLD)+"66",fontFamily:"'Cinzel',serif",fontSize:16}}/>
      }
      <div style={{display:"flex",gap:6,marginTop:6}}>
        <button className="rv-btn-gold" onClick={ok} style={{background:accent||GOLD,color:BG}}>Desar</button>
        <button onClick={cancel} style={{fontSize:16,padding:"4px 12px",borderRadius:3,background:"transparent",color:"#6a6050",border:`1px solid ${BORDER}`,cursor:"pointer",fontFamily:"'Cinzel',serif",letterSpacing:"0.06em"}}>Cancel·lar</button>
      </div>
    </div>
  );
}
...
function Sel({value, opts, onChange, cm}) {
  ...
  return (
    <select autoFocus value={value}
      ...
      onBlur={()=>setEd(false)}
      style={{fontSize:16,padding:"3px 8px",borderRadius:3,border:`1px solid ${GOLD}66`,
        background:"#161410",color:"#e0d4b8",fontFamily:"'Crimson Text',Georgia,serif",cursor:"pointer"}}>
...
function Rule({color}) {
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:8,margin:"10px 0"}}>
      ...
      <span style={{fontSize:16,color:c,opacity:0.5}}>✦</span>
  ...
  const Detail = item ? (
    <div className="rv-detail" style={{flex:1,overflowY:"auto",padding:"18px 20px",position:"relative",
      ...
      {mob&&<button onClick={()=>setShowD(false)} style={{marginBottom:14,background:"none",border:"none",
        cursor:"pointer",color:GOLD,fontSize:16,padding:0,fontFamily:"'Cinzel',serif",letterSpacing:"0.06em"}}>
        ← TORNAR
      </button>}
      ...
      <div style={{display:"flex",gap:12,alignItems:"flex-start",marginBottom:8}}>
            ...
            {tab==="chars"?(
              <>
                ...
                <button onClick={()=>updC(origI,"tags",[...(item.tags||[]),"Guildless"])}
                  style={{fontSize:16,padding:"2px 7px",borderRadius:10,border:`1px dashed ${GOLD}44`,
        ...
        <div style={{display:"flex",gap:4,flexShrink:0}}>
          <button onClick={del} style={{background:"none",border:`1px solid #c4303033`,borderRadius:3,
            cursor:"pointer",color:"#c43030",fontSize:16,padding:"3px 7px",fontFamily:"inherit",
          ...
          {!mob&&<button onClick={()=>{setSel(null);setShowD(false);}}
            style={{background:"none",border:"none",cursor:"pointer",color:"#9a8a70",fontSize:22,padding:"0 2px",lineHeight:1,transition:"color 0.15s"}}
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",paddingLeft:4,marginBottom:4}}>
        {tab==="chars"?(
          <div style={{fontSize:16,padding:"3px 10px",borderRadius:12,
            background:BG2,border:`1px solid ${BORDER}`,
            display:"flex",gap:5,alignItems:"center"}}>
            <span style={{opacity:0.5,fontSize:16}}>🎮</span>
            <span style={{color:"#9a8a70",fontFamily:"'Cinzel',serif",fontSize:16,letterSpacing:"0.06em"}}>CONTROL</span>
            <span style={{color:"#e8dfc8",fontFamily:"'Crimson Text',serif",fontSize:16}}>{item.controlledby}</span>
          </div>
        ):(
          <div style={{fontSize:16,padding:"3px 10px",borderRadius:12,
            ...
            display:"flex",gap:5,alignItems:"center"}}>
            <span style={{opacity:0.5,fontSize:16}}>⚔</span>
            <span style={{color:"#9a8a70",fontFamily:"'Cinzel',serif",fontSize:16,letterSpacing:"0.06em"}}>PROPIETARI</span>
            <span style={{color:ac,fontFamily:"'Crimson Text',serif",fontSize:16,fontWeight:600}}>{item.propietari}</span>
          </div>
        )}
        <div style={{fontSize:16,padding:"3px 10px",borderRadius:12,
          background:BG2,border:`1px solid ${BORDER}`,
          ...
          <span style={{color:"#9a8a70",fontFamily:"'Cinzel',serif",fontSize:16,letterSpacing:"0.06em",flexShrink:0}}>VISIBLE</span>
          <div style={{flex:1,minWidth:0}}>
      ...
      <div style={{marginBottom:8}}>
        <div style={{fontSize:16,color:ac,fontFamily:"'Cinzel',serif",letterSpacing:"0.12em",
      ...
      {/* Notes DM */}
      <div>
        <div style={{fontSize:16,color:"#9a8a70",fontFamily:"'Cinzel',serif",letterSpacing:"0.12em",
      ...
      <div className="rv" style={{height:"100vh",maxHeight:720,
        ...
        <div style={{background:BG2,borderBottom:`1px solid ${BORDER}`,padding:"12px 16px 0",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            ...
            <div>
              <span className="rv-head" style={{fontSize:mob?13:15,color:"#f0e4b0",letterSpacing:"0.07em",fontWeight:600}}>
                Arxiu de Ravnica
              </span>
              {!mob&&<span style={{fontSize:16,color:"#8a7a60",fontStyle:"italic",
                fontFamily:"'Crimson Text',serif",marginLeft:10}}>
                Decàleg del Pacte del Gremi
              </span>}
            </div>
            {saved&&<span style={{fontSize:16,color:GOLD,marginLeft:4,fontFamily:"'Cinzel',serif",letterSpacing:"0.06em"}}>{saved}</span>}
            <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center"}}>
              {!mob&&<span style={{fontSize:16,color:"#8a7a60",fontFamily:"'Cinzel',serif",letterSpacing:"0.05em"}}>
          ...
          <div style={{display:"flex",gap:1}}>
            {[{id:"chars",icon:"👤",label:"Personatges"},{id:"handouts",icon:"📜",label:"Handouts"}].map(t=>(
              <button key={t.id} onClick={()=>chTab(t.id)} style={{
                ...
                padding:"6px 16px",fontSize:16,cursor:"pointer",
        ...
        <div style={{display:"flex",flex:1,overflow:"hidden",position:"relative",background:BG}}>
          ...
          <div style={{
            ...
            <div style={{padding:"8px 10px",borderBottom:`1px solid ${BORDER}`,flexShrink:0,background:BG2}}>
              <div style={{position:"relative",marginBottom:7}}>
                <span style={{position:"absolute",left:9,top:"50%",transform:"translateY(-50%)",
                  fontSize:16,color:GOLD,opacity:0.6,pointerEvents:"none"}}>⚜</span>
            ...
            <div style={{overflowY:"auto",flex:1}}>
              {fltd.length===0&&(
                <div style={{padding:30,color:"#8a7a60",fontSize:16,textAlign:"center",
              ...
              {fltd.map((x,i)=>{
                ...
                return (
                  <div key={i} onClick={()=>pick(i)} className="rv-row" style={{
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:16,color:isSel?"#d4c89a":"#c8bfa8",
                        overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",
                        fontFamily:"'Cinzel',serif",letterSpacing:"0.03em",fontWeight:isSel?600:400}}>
                        {x.nom}
                        {PJS.includes(x.nom)&&<span style={{marginLeft:5,fontSize:16,
                      ...
                      <div style={{display:"flex",gap:3,marginTop:3,flexWrap:"wrap"}}>
                        {tab==="chars"
                          ?(x.tags||[]).map(t=><span key={t} style={{fontSize:16,padding:"1px 5px",
                              borderRadius:8,background:GC[t]+"15",color:GC[t],
                              fontFamily:"'Cinzel',serif",letterSpacing:"0.03em"}}>
                              {GS[t]} {t}
                            </span>)
                          :x.propietari&&<span style={{fontSize:16,padding:"1px 5px",
                        ...
                        {x.visible&&x.visible!=="Tots"&&(
                          <span style={{fontSize:16,padding:"1px 5px",borderRadius:8,
                            background:BG3,color:"#8a7a60",fontFamily:"'Crimson Text',serif",fontStyle:"italic"}}>
                            👁 {x.visible==="Només DM"?"DM":x.visible.split(",")[0].trim()+(x.visible.includes(",")?"…":"")}
                          </span>
                        )}
                      </div>
                    </div>
                    <span style={{color:isSel?c:"#2a2520",fontSize:16,flexShrink:0,transition:"color 0.1s"}}>›</span>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{padding:"8px 10px",borderTop:`1px solid ${BORDER}`,background:BG2,flexShrink:0}}>
              <button onClick={tab==="handouts"?newH:newC} style={{
                width:"100%",padding:"6px",fontSize:16,
                border:`1px dashed ${GOLD}30`,borderRadius:3,background:"transparent",
                color:"#8a7a60",cursor:"pointer",fontFamily:"'Cinzel',serif",
